...
class ContactController
{
    ...
    {
        $find = $this->fmdb->newFindAnyCommand('allContact');
        $find->addFindCriterion('contacemailtID', '*');
        // $per_page = 5;
        // $start =0;
        // $find->setRange($start,$per_page);
        $count=0;
        ...
        foreach($recs as $record){
          ...
          $res['recordId'] = $record->getRecordID();
            foreach ($field as $field_name) {
                if($field_name == 'dob'){
                    $date =  $record->getField($field_name);
                    $newDate = date("Y-m-d", strtotime($date));
                    $res[$field_name] = $newDate;
                }else{
                $res[$field_name] = $record->getField($field_name);
                }
            }
            $response1[$count] = $res;
            $count++;
