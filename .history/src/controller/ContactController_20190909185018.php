...
class ContactController
{
    ...
    public function getRecordOnRange($request, $response, $args)
    {
        ...
        if ($id) {
            ...
            $response1=(['error' => true, 'message' => 'Unauthorized Access.'],
            UNAUTHORIZED_USER);
        }
        return $response->withJson($response1);

    }
    public function getTotalRecord($result, $response)
    {
        $layoutName = "allContact";
        $fmquery = $this->fmdb->newFindCommand($layoutName);
        $fmquery->addFindCriterion('email', '*');
         ...
         */
        $total = $result->getTableRecordCount();
        return $response->withJson(["count" => $total]);
